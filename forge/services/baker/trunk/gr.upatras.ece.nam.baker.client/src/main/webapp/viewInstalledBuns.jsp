...
<body>
  	...
  	<div class="page-header">
		<h1>Installed Buns <br>
		<small>View status and manage installed Buns in your resource</small>
			</h1>
	</div>
    ...
    function loadData(){
	  $.getJSON(bakerURLatFORGEBoxInstall+'/services/api/client/ibuns/', function(data) {
	      var output="<table class='table table-bordered table-condensed ' >"+ 
	    	   ...
	    	   "<th>action</th>" +
	    	 	"</tr>";
	        for (var i in data) 
		        if ((data[i].status!='FAILED') && (data[i].status!='UNINSTALLED')){
    ...
    function setButtonText(vid, vstatus){
    	...
    	if (vstatus === 'INIT'){
    		...
    		$("#execBtn"+vid).text('INSTALL');
    		$("#execBtn"+vid).addClass('btn-info').attr('NEXTACTION', 'INSTALL');		
        	$("#execBtn"+vid).prop('disabled', false);//use can install only from marketplace view
        	$("#execBtn"+vid).hide();
    	}
